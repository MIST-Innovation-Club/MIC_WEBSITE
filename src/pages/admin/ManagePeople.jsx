...
const empty = { name: "", role: "", category: "Executive Panel", bio: "", imageUrl: "", order: 0 };
          ...
          {data.map((p) => (
            <div key={p.id} className="flex items-center gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-400">{p.role} · {p.category}</p>
              </div>
              <button onClick={() => setForm({ ...p })} className="p-2 text-ink-400 hover:text-circuit"><Pencil className="h-4 w-4" /></button>
        ...
        <div className="fixed inset-0 z-[90] bg-ink-950/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setForm(null)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="card p-6 md:p-8 w-full max-w-md max-h-[90vh] overflow-y-auto space-y-4">
            ...
            <div>
              ...
              <input required className="input-field" value={form.role} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))} />
            </div>
            <div>
              <label className="label-field">Panel / Category</label>
