...
const empty = { name: "", role: "", department: "", bio: "", imageUrl: "", order: 0 };

export default function ManageRegulatoryBody() {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (id) {
        await updateDoc(doc(db, "regulatoryBody", id), payload);
      } else {
        await addDoc(collection(db, "regulatoryBody"), { ...payload, createdAt: serverTimestamp() });
      }
      setForm(null);
    } catch (err) {
      console.error(err);
  ...
  const remove = async () => {
    if (!toDelete) return;
    await deleteDoc(doc(db, "regulatoryBody", toDelete.id));
    setToDelete(null);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-display font-semibold text-3xl text-ink-100">Regulatory Body</h1>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState title="No members yet" text="Add regulatory body members so they show up on the Regulatory Body page." />
      ) : (
      ...
      {form && (
        <div className="fixed inset-0 z-[90] bg-ink-950/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setForm(null)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="card p-6 md:p-8 w-full max-w-md max-h-[90vh] overflow-y-auto space-y-4">
            ...
            <ImageUploader folder="regulatory-body" value={form.imageUrl} onUploaded={(url) => setForm((f) => ({ ...f, imageUrl: url }))} />
            <div>
            ...
            <div>
              ...
              <input required placeholder="Chairperson, Advisor…" className="input-field" value={form.role} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))} />
            </div>
            <div>
              <label className="label-field">Department (optional)</label>
              <input placeholder="e.g. Faculty Advisory" className="input-field" value={form.department} onChange={(e) => setForm((f) => ({ ...f, department: e.target.value }))} />
            </div>
            <div>
      ...
      <ConfirmDialog
        ...
        text={`"${toDelete?.name}" will be removed from the Regulatory Body page.`}
        onConfirm={remove}
